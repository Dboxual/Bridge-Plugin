...
```
DISABLED → (enable) → WAITING → IN_GAME → RESETTING (FAWE end-of-match) → WAITING (loop)
```
...
Bridge signs are **permissionless** — any player can click a registered queue sign with no permission required.

| Permission | Default | Purpose |
|---|---|---|
| `bridge.admin` | op | All setup and admin commands (`/bridge create`, `setsign`, `removesign`, `enable`, `disable`, `reload`, etc.). |

`bridge.play` and `bridge.use` do **not** exist in `plugin.yml` and are not checked anywhere at runtime.

`SignListener.onInteract` runs at `EventPriority.HIGHEST, ignoreCancelled = false` to fire after protection plugins (e.g. WorldGuard) and process registered Bridge signs even when the event was pre-cancelled. The event is always re-cancelled afterward to prevent vanilla sign editing. Debug logging (all sign-click decisions) is emitted to console when `settings.debug: true`.
